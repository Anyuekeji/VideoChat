...
@interface ANVideoManager ()<AVCaptureFileOutputRecordingDelegate>
{
...
- (void)startRecordingVideoWithFileName:(NSString *)videoName
{
    ...
    if (!connection) {
        AYLog(@"capture connection wrong!");
        return;
    }
...
- (CGFloat)getFileSize:(NSString *)path
{
    ...
    AYLog(@"file size : %f",[outputFileAttributes fileSize]/1024.0/1024.0);
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error
{
    ...
    [self compressVideo:[mp4Url path] finished:^(NSString *path) {
        ...
        // 删除原录制的文件
        if ([LEFileManager isFileExistsAtPath:outputFileURL.path]) {
            [LEFileManager deleteFileAtPath:outputFileURL.path];
        }
    }];
    // 直接转大小不会改变
...
{
    NSString *videoPath   = [[ANVideoManager shareManager] videoPathWithFileName:[[namePath lastPathComponent] stringByDeletingPathExtension]];
