 ...
 //   [requestHeader addValue:[OpenUDID value]
   ...
   //                  forKey:@"appkey"];
    ...
    [requestHeader addValue:[ZWDeviceSupport platformString]
                     forKey:@"deviceName"];
    //网络环境
   // [requestHeader addValue:[AYUtitle currentReachabilityString] forKey:@"gsm"];
    //客户端｜服务器版本号
...
+ (NSDictionary *) httpHeaderWithReqeustParams:(NSDictionary *)parameters {
    ...
    NSMutableDictionary * requestHeader = [NSMutableDictionary dictionary];
   [requestHeader addValue:@"ae5244896fbf9a3c61123949daf7ff62"
                     forKey:@"apiKey"];
    //组合
    NSTimeInterval timeInterval = [[NSDate date] timeIntervalSince1970];
    ...
    [requestHeader addValue:[AYUtitle getDeviceUniqueId]
             forKey:@"deviceToken"];
    if ([ANUserManager isUserLogin] && ![parameters.allKeys containsObject:@"token"]) {
        [requestHeader addValue:[ANUserManager userItem].myToken
         forKey:@"token"];
    }
//    //
    //拼凑参数MD5token
    ...
    NSString *entst = [AYUtitle hmacStringUsingAlg:kCCHmacAlgMD5 withKey:@"e0fde81efb26cff806ad42ef4ddbb257" str:willSignData];
